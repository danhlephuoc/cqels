...
public class TupleMapping extends MappingBase {
	...
	public boolean containsKey(Object key) {
		if(var2Idx.containsKey(key)) return true;
		return super.containsKey(key);
	}
	
    @Override
    public boolean containsValue(Object value) {
    	if(tuple.asList().contains(value)) return true;
    	//TODO : check if tuple contain value
    	return super.containsValue(value);
    }
    @Override
    public boolean isEmpty() {
    	if (tuple.size()>0) return true;
    ...
    public boolean isCompatible(Mapping mapping) {
    	...
    	for(Entry<Var, Integer> entry:var2Idx.entrySet()){
    		...
    		if(mapping.get(entry.getKey())>0 && mapping.get(entry.getKey())!= tuple.get(entry.getValue()).getId()) return false;
    	}
